...
void lcd_init() {
   ...
   lcd_command(0x21); // set extins extended instruction set
   lcd_command(0xc8); // Vop  v1: 0xc8 (for 3V), v2: 0xa0 (for 3V)
   lcd_command(0x12); // bias (contrast)
   lcd_command(0x20); // horizontal mode from left to right, X axe are incremented automatically,
                              // 0x22 for vertical addressing, back on normal instruction set too
   lcd_command(0x09); // all on

   delay_ms(50);

   lcd_clear();       // reset DDRAM, otherwise the lcd is blurred with random pixels

   delay_ms(10);

   lcd_command(0x08); // mod control blank change (all off)
   delay_ms(10);
